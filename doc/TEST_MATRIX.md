...
`test/catalog_test.dart` is the A-to-Z gate. It shares no helper with the other
test files and re-derives geometry and ownership from the written specification
...
> at any covered point the visible owner is the covering snake whose head tip is
> the closest one behind that point in the drawing direction.

`test/geometry_test.dart`, `test/raster_test.dart`, and `test/widget_test.dart`
are the older suite. They are kept because they cover the painter's internals
and the widget's lifecycle, but they are not the gate: a regression must be
caught by the catalog. `example/test/demo_app_test.dart` covers the example
app's own wiring and runs from the example directory.
...
- Cap radius is exactly half the band at every band fraction, and the silhouette
  closes on the outer circle.
- The independent ring derivation agrees with `WovenRingGeometry` on band,
  track, and joint lag.
- Fractions stay finite, ordered, non-negative, and sum to one for empty,
  ...
  minimum policies.
- Enforcement lifts every positive value to the minimum and is continuous across
...
Fifty-two configurations, each checked for full band coverage, an exact
two-circle silhouette with an untouched hole, and the spec's owner at every
unambiguous pixel on a five-radius polar grid:

- One, two, three, four, ten, and twelve snakes; equal, unequal, tiny-enforced,
...
- Short segments, which is where a covering run can close the whole cycle and
  leave the successor mask with no answer: two and three snakes at 90/10 and
  80/10/10 in every rotation, and a snake past 88 percent, where its own head
  and tail caps touch and its outline self-intersects.
- Band at its minimum, default, maximum, and both clamps. Overlap at 25, 30, 50,
  90, and 100 percent, including thin and wide bands at 90 percent.
- Solid, along-length gradient, tail-to-head gradient, across-band gradient, and
  mixed fills. No border, all borders, mixed borders, diagnostic wide borders,
  darker-fill borders, and selection. Head lift. Dark surface.

Then, measured rather than assumed:

- Every visible colour boundary sits on a circle of radius `band / 2` centred on
  the successor head's cap centre, sampled at eighteen radii across the band.
- Nothing paints outside the silhouette, measured below the pixel. The ring
  ...
  every one of them, over both single-value styles, both directions, both band
  clamps, an off-axis start, borders plain and diagnostic, plain and bordered
  four-snake rings, a self-lapping snake, and every frame of a four-to-one
  merge. The margin allowed is a quarter of a logical pixel, which is the
  antialiasing of the true circles and nothing more.
- A snake that laps its own tail joins with that same cap circle and shows no
...
- Along-length gradients run head to tail the right way round for both gradient
  directions and both senses of rotation; across-band gradients have no angular
  variation at all and do shade across the band.
...
- A single 100 percent snake fills the whole annulus; jointed keeps a hairline
  self-joint, smooth keeps none. Smooth is the default, and the default is
  pinned by its own check that walks seven radii across the whole band, so a
  seam mark cannot come back by way of a changed default.
- All-zero data falls back to the empty track without painting any snake.

## 4. Motion (catalog sections E, F)

- Relay and bloom, CW and CCW, settle bit-for-bit onto the static rendering, so
...
- Every free end of a growing ring is a true cap circle, both ends of every
  covered run, sampled through completion in both directions and both intros.
- A self-lapping snake buries no border: every border pixel has background
...
- Fourteen data moves in both directions, including value changes, four to ten,
  ...
  one to two, three to two, to-zero and from-zero, keep the band covered at
...
- Joint ownership holds through the whole transition for flat, gradient,
  bordered, and lifted rings in both directions, checked by colour-run order
...
- Reduced motion completes the intro and every data change immediately.
- Disposal leaves no live ticker in any mode.
- Aggregate and per-snake semantics are both reachable, and adding a per-snake
...
and direction cross-product at one, two, four, and ten snakes, the short-segment
cases in both rotations, every state, and six frames through each intro. Montage
